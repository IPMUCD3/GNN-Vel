...
def test(loader, model, params_dataimport, params_nn,bias=None):
    model.eval()
    outs = np.zeros((1))
    trues = np.zeros((1))
    yerrors = np.zeros((1))
...
# Training procedure
def training_routine(model, train_loader, valid_loader, params_dataimport, params_nn, verbose=True):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        valid_loss, err = test(valid_loader, model, params_dataimport, params_nn)
...
def correct_bias(params_dataimport, params_nn):
